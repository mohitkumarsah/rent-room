...
var roomsRelations = relations(rooms, ({ one }) => ({
  owner: one(users, {
...
var roomsRouter = router({
  generateDescription: publicProcedure.input(
    z2.object({
      rent: z2.number().positive(),
      type: z2.string(),
      amenities: z2.array(z2.string()),
      location: z2.string()
    })
  ).mutation(async ({ input }) => {
    try {
...
The description should be 2-3 sentences, highlight key features, and be appealing to potential tenants. Write in a professional yet friendly tone.`;
      const response = await invokeLLM({
        messages: [
          {
            role: "system",
            content: "You are a professional real estate copywriter. Generate compelling room listing descriptions that highlight key features and appeal to renters."
          },
          {
            role: "user",
            content: prompt
...
function writeToLogFile(source, entries) {
  ...
  const lines = entries.map(
    (entry) => `[${(/* @__PURE__ */ new Date()).toISOString()}] ${JSON.stringify(entry)}`
  );
